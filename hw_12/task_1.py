...
class Student:
    def __init__(self, name, subjects_file):
        ...
        self.__setattr__('name', name)  # вызываем дескриптор при создании экземпляра
        self.subjects = {}
        # Загружаем предметы из файла
        self.load_subjects(subjects_file)  # загружаем в словарь subjects предметы из файла

    def __setattr__(self, name, value):
        # Проверяем ФИО на правильность
        if name == 'name':  #  проверка, что работаем с атрибутом name
            # проеверяем: имя не пустое, имя начинается с заглавной буквы, имя состоит из букв
    ...
    def load_subjects(self, subjects_file):
        try:
            ...
            with open(subjects_file, newline='', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    # для каждого элемента в строке удаляем пропуски и сохраняем в список
                    subjects = [subject.strip() for subject in row]
                    for subject in subjects:
                        if subject:
                            # каждый элемент из списка сохраняем как ключ в словаре который инициализируется в __init__
                            # в значение сохраняем другой словарь содержащий ключи grades и test_scores
    ...
    def add_grade(self, subject, grade):
        ...
        return self.subjects[subject]['grades'].append(grade)
    ...
    def add_test_score(self, subject, test_score):
        ...
        return self.subjects[subject]['test_scores'].append(test_score)
    ...
    def get_average_grade(self):
        ...
        # из каждого subjects ключа берем значение, затем обращаемся к каждому значению(которое является словарем)
        # и берем из него значение по ключу 'grades' и сохраняем в список List Comprehension
        # если результирующий спосок пуст возвращаем 0,0
